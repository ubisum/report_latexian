...
Within the wide field of robotics, mobile robots gained much attention in recent years, because of the great ambit in which they can be involved in (Figure 1.1). Just to mention some of the areas of interest in which mobile robots are requested to operate, consider animation of digital characters, space exploration, mapping of and navigation in unknown and/or changing environments. \\
...
\centerline{\includegraphics[width=0.6\textwidth]{images/nao.jpg}}
(a)
\end{minipage} \\
\vspace{10mm}%
\begin{minipage}[c]{0.9\textwidth}
\centering\setlength{\captionmargin}{0pt}%
\centerline{\includegraphics[width=0.6\textwidth]{images/rescue.jpg}}
(b)
\end{minipage} \\
\vspace{10mm}%
\begin{minipage}[c]{0.9\textwidth}
\centering\setlength{\captionmargin}{0pt}%
\centerline{\includegraphics[width=0.6\textwidth]{images/quadrotor.jpg}}
(c)
\end{minipage}
\caption{Three examples of mobile robots: the Nao humanoid robot (a), a robot used for rescuing operations (b) and a quadrotor (c)}
\end{figure}

Deriving the shape of ambient is not the only result that can be obtained by analysing measurements returned by sensors; indeed, such data, can be exploited to reconstruct the set of poses assumed by robot in all the spots where it used sensors to inspect environment; this furnishes a track of robot and sensors (in terms of position in space and orientation) in time. \\
...
A lot of research has been done in last decades to perform robot tracking and environment mapping by maximising the outcome of sensors' acquisitions. In particular, we can mention two specific fields that particularly took advantage from research in tracking and mapping procedures: \\
\begin{itemize}
 	\item{\textbf{visual SLAM (VSLAM)}, i.e. the problem of building a sparse or dense 3D model of a scene while traversing it, determining, at the same time, the trajectory of robot's platform and of a camera mounted on it}
	\item{\textbf{scan matching}, the process of computing the rigid transformation that best aligns two laser scans or maps}
\end{itemize}
In any case, all the algorithms that mean to deal with tracking and mapping tasks are required to be fast and robust, in order to reduce error to minimum and construct a reliable estimation of robot's track in real time.
