...
ANTOINE DE SAINT-EXUPERY, \emph{The little prince}
\end{flushright}
...
\begin{flushright}
\emph{
To my grandfather, who would have seen this day, if he had had still grains to flow through his hourglass.\\
To Gaia, for the enduring patience and the infinite love, two fundamental requisites when you begin to write a thesis. \\
To my parents and the rest of the family crew, for believing that, one day, I would have done it.
}
\end{flushright}
...
An alternative formulation can be obtained for point features using \emph{iterative closest point (ICP)}, if an initial set of association is known; in this case, minimising a squared error function, it is possible to retrieve sought transformation in closed form. When initial set of correspondences is not available at the beginning, it can be constructed using a random sampling of points or using a data association policy (for example, coupling each point to its nearest neighbour).\\
Anyway, dense approaches like ICP reveal to be very slow in computing the transformation in closed form and require a good initial guess. Moreover, they are too sensible to sudden changes in observed scenes. On the other hand, feature-based algorithms perform quickly, since they work on a subset of data and are insensible to scene changes, since such changes are simply ignored. \\
\begin{figure}[htb]
\centering
\centering\setlength{\captionmargin}{0pt}%
\vspace*{0.06in}
...
\end{figure}
In this thesis we will introduce a new method to solve scan matching issue, presenting it as a least square estimation problem. We validated our approach with a systematic set of experiments. The results show that our method is fast, effective, robust and adequate for on-line operations.
...
\end{equation}
in which $\theta$ expresses the direction of line's normal vectors and $\rho$ its distance from origin (see Figure 2.1), we can select a family of sets to apply HT:
\begin{equation}
...
\end{equation}
Computing the HT passes through a discrete approximation of it, called \emph{Discrete Hough Transform (DHT)}; the comparison with simple HT is shown in Figure 2.2. DHT allows to redefine our family of sets as:
\begin{equation}
...
\end{figure}
For a complete definition of the scan matching algorithm based on HT, we need to define the so called \emph{Hough spectrum}. For such purpose, we need a functional $g$ that is invariant to translation. Theoretically speaking, this means that, given a function $f$:
\begin{equation}
...
\end{equation}
An interesting property of spectrum is its \emph{invariance}. If we define again two inputs $i(\bf{s})$ and $i'(\bf{s})$ as in (2.5) and (2.6), the following statement is valid:
\begin{equation}
...
The scan matching problem is almost solved; what remains to be done is the estimation of translation $T$. Let's assume $\phi=0$ (this can be obtained by shifting the columns of $\mbox{DHT}^S$ and $\mbox{DHT}^R$ by the $\phi$ previously computed). We can choose an arbitrary $\theta = \hat{\theta}$. We can derive that
\begin{equation}
...
\begin{algorithm}
	...
	\end{algorithmic}
\end{algorithm}
The main tasks performed by algorithm are extracting lines from current scan and comparing them with the ones contained in a \emph{a priori} map. In particular, function \emph{SelectScanline} picks a a new line to be matched, while \emph{VerifyMatch} checks that a new pair \emph{(m, s)} of matched lines is compatible with the pairs already accepted in previous iterations of procedure. Finally, the algorithm returns a set of accepted pairs, that can be translated into positions from which robot may have taken scans (Figure 1.3). Proper constraints on lines' lengths and roto-traslations can allow algorithm to be more effective; this implies that it is not necessary to have a good estimate of robot's initial pose, since robot is able to recover from initial error.  
\begin{figure}[htbp]
\centering
\centering\setlength{\captionmargin}{0pt}%
...
The equation (2.21) is not satisfied completely, then finding the six unknown motion parameters means to find $A$ and $\boldsymbol{d}$ that minimise dissimilarity
\begin{equation}
	\epsilon = \int \int_W [J(A\boldsymbol{x} + \boldsymbol{d} - I(\boldsymbol{x})]^2 w(\boldsymbol{x})d\boldsymbol{x}
\end{equation}
with $W$ the feature window and $w(\boldsymbol{x})$ a weighting function (in the simplest case, it can be set to 1. In the other cases, it can be a Gaussian-like function).

When we have a simple translation, matrix $A$ must coincide with identity matrix. To minimise the value in equation (2.23), it is possible to differentiate with respect to unknown parameters in $D$ and $\boldsymbol{d}$ and then set result to zero. The obtained system can linearised by a truncation in Taylor expansion:
\begin{equation}
...
First of all, we will concentrate ourselves on describing the concept of sensors and their importance in self-perception of robots. Sensors, indeed, are very useful to allow a robot to localise itself and have an idea of surrounding environment; anyway, to make all this possible, it is necessary to design them in such a way to satisfy a series of requirements. Sensors, although their immense utility, furnish noisy measurements with which localisation algorithms must deal.
In this chapter, we will describe one particular kind of sensors, the ones based on the \emph{time of flight (TOF)} method.

The main technique used by robots to estimate their current position and orientation is \emph{odometry}, which is based on data returned by robot's encoders. We will analyse the two types of errors it introduces and how they have been experimentally measured.
...
Finally, we will propose a process based on least squares minimisation, that allows to estimate a robot's state (in terms of position and orientation), exploiting the error between measured and expected state value.
...
As it is evident in (3.1), the range to an object can be easily retrieved and no further knowledge about object's nature is necessary.
...
Slippage is only one of the possible causes of errors in odometry; they can me summarised and divided into the following two categories:\\ \\
...
As explained in previous section, data retrieved from odometry are often inaccurate and they can lead to errors in robot's position estimation; features, after their extraction from sensors' data, can add a precious information allowing to reduce the errors and gain a better estimate of robot position in time. Indeed, each feature carries information about its reciprocal position with respect to robot and it can be used to improve localisation methods\\
...
The main steps of procedure are described by Algorithm 2. In particular:
\begin{itemize}
...
\begin{equation}
	\{f_i(\textbf{x})\}_{i = 1:n} \notag
\end{equation}
which, given \textbf{x}, predict its measurement. \\Let's suppose, moreover, to have a series of real measurements ${\bf z_i}$. The goal is to estimate the state ${\bf x}$ that best fits the measurements ${\bf z_{i:n}}$.\\
...
If a good initial guess is at disposal and the measurement functions are smooth in the neighbourhood of minima, we can solve problem using iterative (local) linearisations. The iterative process is composed of three steps:
\begin{itemize}
...
\end{itemize}
The first step is linearising the problem and this can be done using a Taylor expansion:
\begin{align}
...
\begin{itemize}
	\item{linearize the system around current guess $\textbf{x}$ and compute $\textbf{e}_i(\textbf{x} + \Delta\textbf{x})$ as described in (3.15) and (3.16)}
	\item{compute $\textbf{b}^T$ and $\textbf{H}$ as described in (3.18) and (3.19)}
	\item{solve the system to get a new optimal increment $\Delta\textbf{x*}$, as indicated in (3.21)}
...
The problem of determining $T$ can be formulated in terms of a least square estimation, by exploiting the concepts in Section 3.5. Let's suppose that a generic segment $l_i$ in scan $i$ can be expressed as 
\begin{equation}
	l_i = \left(\begin{matrix} p_i \\ n_i \end{matrix} \right)
\end{equation}
where $p_i$ is a point on segment (midpoint is the most convenient choice) and $n_i$ is segment's normal, in its classical expression
\begin{equation}
...
\textbf{Computing local and global matrices}. Once we have a set of reliable couples of indexes (lines), for each couple we can compute the error $e_{ij}$ as expressed in Equation (4.9); this allows us to determine the local matrices $H_{ij}$ e $b_{ij}$ (Equations from 4.9 to 4.14), for each couple of associated lines. Once all local matrices have been computed, we can derive global matrices $H$ and $b$ (Equations 4.15).

\textbf{Solving linear system and updating current estimate of $T$}. Once the global matrices have been computed, it is possible to prepare linear system in (4.16). Its resolution gives us the new estimate $\Delta T$ of transformation matrix between scans $i$ and $j$.  By applying new estimate to old one, we get a new transformation matrix that can be used for a new iteration of algorithm or can be directly returned if the maximum number of iterations has been reached. A block representation of a single iteration can be found in Figure 4.2.
...
\begin{itemize}
	...
	\item{robot-specific features: pose estimation, localisation, mapping, navigation (Figure 5.1)}
	...
	\item{tools for debugging, plotting, and visualizing the state of the system under development (Figure 5.2)}
...
\begin{itemize}
	\item{a \emph{.png} file representing the shape of environment, created with the use of a laser rangefinder or by graphics editors (Figure 5.3)}
	\item{a \emph{.world} file, containing the description of ambient (dimension of map, robots' initial positions and colors). An example relative to Figure 5.3 is showm in Figure 5.4}
	\item{a \emph{.yaml} file, containing information, for instance, regarding the position of origin of axes on \emph{.png} file, that will be used as a reference to track robot's position along its trajectory (Figure 5.5)}
\end{itemize}
\begin{figure}[htbp]
\centering
...
\caption{A \emph{.world} file relative to Figure 5.3}
\end{figure}
...
\caption{A \emph{.yaml} file relative to Figure 5.3}
\end{figure}
...
\end{equation}
The quantities in (5.3) computed at $i$-th iteration of LSE can be summed, gaining an error measurement $\chi_i$. Thus, after $N$ iterations, LSE returns a total local error
\begin{equation}
	\chi = \sum_i^N \chi_i
\end{equation}
...
\chapter{Conclusions}
Along this thesis, we have presented a new method to solve the problem of scan registration, on which many robotics applications (that have necessity to perform tasks involving localisation and mapping) are based. \\
We decided to use a feature-based approach, which extracts part of information and works solely with it; this is the reason why such methods reveal to be faster, if compared to dense methodologies which work with a greater amount of data, require a good initial guess and are too sensible to small changes in observed scene.

As extracted features, we chose lines and designed a representation for them based on midpoints and normals; using this representation, we built a process based on least squares estimation (LSE) that iteratively estimates the rigid transformation to align two different frames, containing lines extracted from their relative laser scans. \\
We prepared some systematic tests by taking a frame, applying a chosen transformation to it and requiring LSE to retrieve the best aligning transformation. As results have shown, the LSE module is perfectly able to determine the transformation that aligns transformed frame with the original one; moreover, by analysing the error accumulated by LSE along the iterations, we observed that it converges rapidly to zero after a few loops. This showed that our method is fast and effective at the same time, as we stated in introduction.

Subsequently, we decided to stress our method with a harder test: instead of using an original frame and the one obtained by applying an initially known transformation to it, we considered two subsequent frames, containing lines extracted from two diverse poses of robot. In this case, LSE by itself is not able to retrieve an acceptable transformation to align the two frames; thus, it needs a new module to furnish it a good set of line correspondences at each iteration. \\
Then, we designed a data association procedure that, on the basis of transformation calculated at last iteration, maps lines from one frame to another and, in the former, finds correspondences on the basis of a threefold criterion, based on normals' similarity, distance between midpoints and differences between slopes. \\
With the introduction of such reinforcement, we were able to determine a good transformation that allows to align the two frames; using such transformation, our algorithm can perfectly compute the right associations between lines, as observed by visual inspection.

Thus, judging our system based on LSE and data association as a whole, we can say that we designed a method that is non only fast and effective (in terms of obtained results and margin of error), but it is even robust and perfectly suitable for on-line operations, as we committed to do in introduction.

Regarding possible future works, our representations of lines and all the work we have described so far can easily be extended to planes. \\
Moreover, the scan registration system, formed by the LSE and data association blocks can be used for the so called \emph{graph SLAM}, which solves the SLAM problem by optimising a graph, whose nodes represent robot's poses or landmarks in the environment and whose edges describe either a transition from one pose to another one or an observation of a landmark from a giver pose. The functionalities of our system allow to perform a correct data association between the scans acquired at two different poses, recognising, thus, if a landmark was observed more that once, in order not to insert it repeatedly. 
